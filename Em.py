...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกัน💢โดย ฟ้า Bot Self ระบบได้ทำการบล็อคคุณอัตโนมัติ",
...
mulai = time.time() 
dangerMessage = ["ฟ้าเสียใจด้วยค่ะ","โชคดีค่ะ","ชีวิตยังมีงวดหน้าเสมอ",".winebot",".kickall","mayhem","kick on","makasih :d","!kickall","nuke","บิน",".???","Kickall","บินไปดิ","Kickword","kickword","!kickword","/kickall"]
dangerMessage2 = ["ว่างัยจ๊ะทิรักของฟ้า 5555","คร้าบบ","ว่างัยน่ะ!!.. รักฟ้าหรออ อิอิ","ว่างัยเอ๋ยย ฟ้ารอฟังอยู่","ฟ้ามาแล้วคร้าบบ","แอบรักฟ้าอยู่หราาา","แอบชอบฟ้าหราา","รักฟ้าหรอ","คิดถึงฟ้ามากเลยหรอ"]
...
def listgrup():
    listGrup =  """💢คำสั่งตั้งค่า ฟ้า Bot Self💢
...
def helpkicker():
    helpKicker = """⏳คำสั่งค้นหา ฟ้าBotSelf⏳
...
def helpsetting():
    helpSetting = """⚓คำสั่งกลุ่ม ฟ้าBotSelf⚓
🍁 กลุ่ม ➠ ข้อมูลกลุ่ม
🍁 ไอดีกลุ่ม ➠ไอดีห้อง
🍁 ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """⚡คำสั่งเลียนแบบฟ้าBotSelf⚡
🔁 เลียนแบบ on/off 
🔁 เลียนแบบ 
🔁 เพิ่มเลียนแบบ (@) 
🔁 ลบเลียนแบบ (@)"""
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    if targets == []:
                        line.sendMessage(to, "💨ไม่มีคนใส่ล่องหนในกลุ่มนี้ครับ✅")
                    else:
                        mc = ""
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendMessage(to, "💠ไม่มี MID คนใส่ล่องหนครับ✅")
                    else:
                        mc = ""
                        for mi_d in lists:
                            mc += "->" + mi_d + "\n"
                    ...
                    ki.sendMessage(to, "ฟ้า BotSelfยังอยู่ครับ")
                elif text.lower() == 'เทส':
                    line.sendMessage(to, "ฟ้า BotSelfกำลังโหลด:▒...0%")
                    ...
                    line.sendMessage(to, " 🔱ฟ้า BotSelf ยังอยู่ครับ✔")
                    ...
                    start = time.time()
                    line.sendMessage(to,"ความเร็วของ ฟ้า Bot Self...")
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["speed","sp","Speed","สปีด","!speed","/speed"]:
                    line.sendText(msg.to,"แรงขนาดนี้ ฟ้านี้ไลค์เลย👍")
                ...
                if msg.text in ["555","5555","55555","55+","55++","555+","555++","5555+","5555++"]:
                    line.sendText(msg.to,"ฟ้าว่าอาการหนักนะเนี๊ยะ!!. อิอิ😝😝")				
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendText(msg.to,"ตรวจพบคำสั่งของบอทลบกลุ่ม  ฟ้า Bot Self จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก")										
        if op.type == 17:
          if settings["Wc"] == True:
                ...
                if RfuCctv['cyduk'][op.param1]==True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref=['สวัสดีครับคนอ่าน➡','ตร๊ะ!..เอ๋!!..😝➡','หวัดดีจร้า💖➡','ฟ้าเห็นนะ อิอิ..😝➡','แอบทำมัย? มาคุยกันหน่อย➡','แอบแฟนเล่นหราา💨➡','ฟ้ารู้นะว่าคัยอ่าน👉➡','เล่นซ่อนแอบหรอคร้าบบ👻➡','มาคุยกับเค้าหน่อย➡','นั่นแน่ แอบหราาา➡']
