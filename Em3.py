...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกัน💢โดย 🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭 ระบบได้ทำการบล็อคคุณอัตโนมัติ\n",
...
def listgrup():
    listGrup =  """🔰คำสั่งตั้งค่า⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🔰
...
def socmedia():
    socMedia = """💠คำสั่งแทค⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣💠
📚คท @ ➠ แทคคนที่จะเอาคท
📚มิด @  ➠ ลงmidคนอื่น
📚ชื่อ @ ➠ ลงชื่อคนอื่น
📚ตัส @ ➠ ลงตัสคนอื่น
...
def helpkicker():
    helpKicker = """📱คำสั่งค้นหา🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭📱
...
def helpsetting():
    helpSetting = """⚓คำสั่งกลุ่ม⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣⚓
🔊 กลุ่ม ➠ ข้อมูลกลุ่ม
🔊 ไอดีกลุ่ม ➠ไอดีห้อง
🔊 ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """💥คำสั่งเลียนแบบ⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣💥
❇ เลียนแบบ on/off 
❇ เลียนแบบ 
❇ เพิ่มเลียนแบบ (@) 
❇ ลบเลียนแบบ (@)"""
...
def helplanguange():
    helpLanguange =    """🌟คำสั่งล่องหน⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🌟
ℹ!แทค
ℹ!มิด
ℹ!คท"""
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    if targets == []:
                        line.sendMessage(to, "ℹไม่มีคนใส่ร่องหนในกลุ่มนี้ครับ✅")
                    else:
                        mc = ""
                        for target in targets:
                    ...
                    for g in gs.members:
                        if g.displayName in "":
                            lists.append(g.mid)
                    if lists == []:
                        line.sendMessage(to, "ℹไม่มี MID คนใส่ร่องหนครับ✅")
                    else:
                        mc = ""
                        for mi_d in lists:
                            mc += "->" + mi_d + "\n"
                    ...
                    ki.sendMessage(to, "🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭ยังอยู่ครับ")
                elif text.lower() == 'เทส':
                    line.sendMessage(to, "🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭กำลังโหลด:▒...0%")
                    ...
                    line.sendMessage(to, "🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭ยังอยู่ครับ✔")
                    ...
                    start = time.time()
                    line.sendMessage(to,"ความเร็วของ🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭...")
...
#==============================================================================#   
                    ...
                    with requests.session() as web:
                        ...
                        ret_ = "╔══[ ผลการค้นหาของ 🇹🇭⍣ᎢᎬᎪᎷᏴᎾᎢ⅌F∀Ħ⍣🇹🇭 ]"
                        datas = []
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["555","5555","55555","55+","55++","555+","555++","5555+","5555++"]:
                    line.sendText(msg.to,"กูว่ามึงอาการหนักนะเนี๊ย!!. อิอิ😝😝")				
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendText(msg.to,"ตรวจพบคำสั่งของบอทลบกลุ่ม  ฟ้า BotSelf จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก")										
        if op.type == 17:
          if settings["Wc"] == True:
                ...
                if RfuCctv['cyduk'][op.param1]==True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref=['สวัสดีคนอ่าน➠','ตร๊ะ!..เอ๋!!..😝➠','แอบทำไมเดียวจิ้มตาบอด..😜➠','เห็นนะ อิอิ..😝➠','แอบทำมัย? มาคุยกันหน่อย➠','แอบแฟนเล่นหราา💨➠','รู้นะว่าคัยอ่าน👉➠','เล่นซ่อนแอบหรอคร้าบบ👻➠','มาคุยกับเค้าหน่อย➠','นั่นแน่ แอบหราาา➠','จร๊ะ!!..เอ๋!..➠']
